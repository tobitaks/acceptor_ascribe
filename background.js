...
function stopMonitoring(reason) {
  chrome.storage.local.get('monitorState', ({ monitorState }) => {
    ...
    console.log(`[Ascribe bg] monitoring stopped — ${reason}`);
  });
}
...
async function runBatchedAccepts(list, batchSize) {
  ...
  if (limit > 0) {
    ...
    if (remaining <= 0) {
      console.log(`[Ascribe bg] daily accept limit ${limit} already reached (${already}) — skipping ${list.length} & stopping`);
      stopMonitoring(`daily accept limit ${limit} reached`);
      return;
    }
    if (list.length > remaining) {
      console.log(`[Ascribe bg] trimming ${list.length} → ${remaining} to stay under daily limit ${limit}`);
      work = list.slice(0, remaining);
    }
  }

  console.log(`[Ascribe bg] accepting ${work.length} order(s) in batches of ${batchSize}`);
  ...
  if (limit > 0) {
    ...
    if (done >= limit) {
      console.log(`[Ascribe bg] daily accept limit reached (${done}/${limit}) — stopping`);
      stopMonitoring(`daily accept limit ${limit} reached`);
    }
  }
}
